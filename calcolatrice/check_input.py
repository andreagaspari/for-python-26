...
def input_operatore(prompt: str) -> str:
    '''
        Verifica la validità dell'operatore della calcolatrice
        (+, -, *, /, **, ^)

        prompt (str): Il testo da mostrare alla richiesta

        str: l'operatore scelto
    '''
    # Usando l'altra funzione (input choice)
    # return input_choice(prompt, ["+", "-", "*", "/", "**", "^"])
    
    while True:
        operatore = input(prompt)
        match operatore:
            case "+" | "-" | "*" | "/" | "**" | "^":
                return operatore
            case _:
